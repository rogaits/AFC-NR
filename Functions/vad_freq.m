function myvad = vad_freq(v, Fs, win, N_STFT, R_STFT,varargin)
    y_STFT   = calc_STFT(v, Fs, win, N_STFT, R_STFT, 'onesided');
    ...
    y_pow = mean(abs(y_STFT).^2, 2);

    VAD = abs(y_STFT).^2 > alpha*(repmat(y_pow, 1, size(y_STFT, 2)));
    myvad= double(VAD);
%     figure; imagesc(VAD);
