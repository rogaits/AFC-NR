function output = rank2_NR_AFC_cl_fvad(filename,iSNR,plotting_flag,alpha,mu_f,nA,MetricsToCompute,R,asg_flag,lambda)
% This functions performs multi-channel noise reduction followed by
% acoustic feedback cancellation for multi-microphone single-loudspeaker
% (MMSL) scenario. 
% It uses an (M+1)-channel MWF follwed by a single-channel PEM-based AFC
...
if asg_flag==0
    ...
    H_mwf = [ones(R/2+1,1) zeros(R/2+1,M-1)]; % Initial MWF filter coefficients
...
% Signal covariance matrices initializations
dimR = N;                               % Covariance signal dimension
...
W = [eye(2); zeros(N-2,2)];         % Initial MWF filter
l = 1;                              % frame index
...
for k = k_init:R/2:Nt
    ...
    for kbin=1:R/2+1
        ...
        if Ryy_initialized(kbin) == 1 && Rnn_initialized == 1
            W = mwf_GEVD2(Ryy(:,:,kbin),Rnn(:,:,kbin),2);       
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        Y_hat(kbin,:) = W'*Y_aux(:,kbin); % MWF filtering for both microphone and loudspeaker signals
        if asg_flag==1
            H_aux= W';
            % selecting the filter coefficients that correspond to the microphones
            H_mwf(kbin,:) = H_aux(2,2:N);   
        end
    end
    % WOLA synthesis
    y_hat_nr = stft_synthesis(l,y_hat_nr,R,Y_hat(:,2),delay_nr+Lf,win);
    u_hat_nr = stft_synthesis(l,u_hat_nr,R,Y_hat(:,1),delay_nr+Lf,win);        
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % AFC stage
...
%           in the AR estimation because the loudspeaker signal is zero
    ...
    if asg_flag==0
        ASG_dB_pem(l) = asg_computation(fTrue,f_hat,delay_fwd,R,msg_dB(idx_ref),idx_max_msg(idx_ref));  
    else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% input SNR check
iSNR = 10*log10(var(v(vad_speaker==1,:))./var(n)) 
...
if plotting_flag==1
    ...
    figure('Name','Rank-2 NR-AFC: Spectrograms')
    subplot(411)
    ...
    figure('Name','Rank-2 NR-AFC: Time-domain signals');hold on;grid on
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    t2 = (Lf:R/2:Nt)/Fs;
    figure('Name','Rank-2 NR-AFC: Mis, ASG, MSE')
    subplot(311)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    figure('Name','Rank-2 NR-AFC: Estimated and True feedback path')
    plot(f_hat,'Displayname','$\hat{f}_r$','LineWidth',1)
